...
- `ingestion_queue_item`  
  - id, document_node_id (FK)
  - status enum: queued | processing | indexed | failed | download_failed  
  - source enum: user | system  
  - added_by_user_id (nullable FK), created_at, updated_at, started_at, finished_at  
  - rag_message (nullable), storage_path (path in `rag_shared_dir`)
